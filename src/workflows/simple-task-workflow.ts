...
export function createSimpleTaskWorkflow(hatchet: HatchetClient, workflowName: string) {
  const workflow = hatchet.workflow<CodingTaskInput>({
    name: workflowName,
    inputValidator: CodingTaskInputSchema,
    description: 'Simple coding task. Executes user input directly without mode parsing.',
    taskDefaults: {
  ...
  const executeTask = workflow.task({
    ...
    fn: async (input: CodingTaskInput, ctx: any) => {
      ...
      await ctx.logger.info(`[Execute] direct mode, workDir=${workDir}`);
      const { steps, summary } = await runClaude({ prompt: input.description, workDir }, log);
      return { mode: 'direct', result: summary, steps, workDir };
    },
  });
  ...
  workflow.task({
    ...
    fn: async (input: CodingTaskInput, ctx: any) => {
      ...
      if (!isWorktree) {
        ...
        return { branch: '', committed: false, result: executeResult.result };
      }
      ...
      return { branch, committed: true, workDir, result: executeResult.result };
    },
  });
